...
func TestDecipheringRunestoneWithTruncatedVarintSucceeds(t *testing.T) {
	...
	builder := txscript.NewScriptBuilder()
	builder.AddOp(txscript.OP_RETURN).AddOp(OP_MAGIC_NUMBER).AddData([]byte{128})
...
func TestOutputsNonPushDataOpcodesAreCenotaph(t *testing.T) {
	builder := txscript.NewScriptBuilder()
	builder.AddOp(txscript.OP_RETURN).AddOp((OP_MAGIC_NUMBER)).AddOp(txscript.OP_VERIFY).
	...
	builder = txscript.NewScriptBuilder()
	builder.AddOp(txscript.OP_RETURN).AddOp((OP_MAGIC_NUMBER)).AddData([]byte{0}).
...
func TestPushNumOpcodesInRunestoneProduceCenotaph(t *testing.T) {
	builder := txscript.NewScriptBuilder()
	builder.AddOp(txscript.OP_RETURN).AddOp((OP_MAGIC_NUMBER)).AddOp(txscript.OP_1)
	script1, err := builder.Script()
	assert.NoError(t, err)
	tx := wire.NewMsgTx(wire.TxVersion)
	tx.AddTxOut(wire.NewTxOut(0, script1))
...
func TestRunestoneMayBeInSecondOutput(t *testing.T) {
	...
	pkScript, _ := txscript.NewScriptBuilder().
		AddOp(txscript.OP_RETURN).AddOp(OP_MAGIC_NUMBER).AddData(payload).Script()
	tx := wire.NewMsgTx(2)
...
func TestAllPushdataOpcodesAreValid(t *testing.T) {
	for i := 0; i < 79; i++ {
		t.Run(fmt.Sprintf("case[%d]", i), func(t *testing.T) {
			...
			scriptPubkey = append(scriptPubkey, uint8(OP_MAGIC_NUMBER))
...
func TestAllNonPushdataOpcodesAreInvalid(t *testing.T) {
	for i := 79; i <= math.MaxUint8; i++ {
		pkScript, _ := txscript.NewScriptBuilder().AddOp(txscript.OP_RETURN).AddOp(byte(OP_MAGIC_NUMBER)).AddOp(byte(i)).Script()
