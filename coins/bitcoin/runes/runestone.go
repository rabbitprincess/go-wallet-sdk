...
const (
	...
	OP_MAGIC_NUMBER      = txscript.OP_13
	COMMIT_CONFIRMATIONS = 6
)

func NewRuneStone() *Runestone {
	return &Runestone{}
}
...
func (r *Runestone) AddEdict(runeId RuneId, amount uint64, output uint32) *Runestone {
	if r.Edicts == nil {
		r.Edicts = make([]Edict, 0, 10)
	}
	r.Edicts = append(r.Edicts, NewEdict(runeId, amount, output))
	return r
}
...
func (r *Runestone) payload(transaction *wire.MsgTx) (*Payload, error) {
	for _, output := range transaction.TxOut {
		...
		if !tokenizer.Next() || tokenizer.Err() != nil || tokenizer.Opcode() != OP_MAGIC_NUMBER {
